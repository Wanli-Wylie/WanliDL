...
class VisionTextMatching(nn.Module):
    ...
    def forward(self, vision_feature, text_feature):
        ...
        text_projected = self.proj(text_feature)  # Output shape: B x projection_dim

        # Ensure both features are in the same shape B x projection_dim
        assert vision_feature.size(1) == text_projected.size(1), "Feature dimensions do not match"

        # Compute similarity by dot product or cosine similarity
        similarity = F.cosine_similarity(vision_feature.unsqueeze(1), text_projected.unsqueeze(0), dim=2)  # B x B
        return similarity
